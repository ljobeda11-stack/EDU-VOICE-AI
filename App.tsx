...
import { Mic, Volume2, BookOpen, GraduationCap, FileText } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (mode) {
      ...
      case AppMode.LESSON_PLANNER:
        return <LessonPlanner apiKey={API_KEY} />;
      case AppMode.HOME:
      default:
        return (
          <div className="flex flex-col items-center justify-center h-full p-6 space-y-12 animate-fade-in py-16">
             <div className="text-center space-y-6 max-w-2xl">
               ...
               <p className="text-xl text-blue-200/80 font-light">
                 Your intelligent teaching companion. Brainstorm lessons, plan curriculum, and create engaging audio content instantly.
               </p>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-6xl">
               <button 
                 ...
                 className="group relative overflow-hidden p-8 glass-panel rounded-2xl glass-panel-hover transition-all duration-300 text-left h-full"
               >
                 <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                   <Mic className="w-32 h-32 text-blue-500 blur-sm" />
                 </div>
               ...
               <button 
                 onClick={() => setMode(AppMode.LESSON_PLANNER)}
                 className="group relative overflow-hidden p-8 glass-panel rounded-2xl glass-panel-hover transition-all duration-300 text-left h-full"
               >
                 <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                   <FileText className="w-32 h-32 text-teal-500 blur-sm" />
                 </div>
                 <div className="w-14 h-14 bg-teal-500/20 border border-teal-500/30 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-lg shadow-teal-500/20">
                   <FileText className="w-7 h-7 text-teal-300" />
                 </div>
                 <h3 className="text-2xl font-bold text-white mb-2">Lesson Planner</h3>
                 <p className="text-gray-400 group-hover:text-gray-300 transition-colors text-sm">
                   Generate structured lesson plans, objectives, and activities for any grade level in seconds.
                 </p>
               </button>
               ...
               >
                 <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                   <Volume2 className="w-32 h-32 text-indigo-500 blur-sm" />
                 </div>
    ...
    <div className="min-h-screen font-sans flex flex-col">
      ...
      <header className="fixed top-0 w-full z-50 glass-panel border-b-0 border-white/5 shadow-lg shadow-black/5 backdrop-blur-xl bg-[#0f172a]/70">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <nav className="flex space-x-1 sm:space-x-2 bg-black/20 p-1.5 rounded-xl border border-white/5">
            <button 
              onClick={() => setMode(AppMode.LIVE_ASSISTANT)}
            ...
            <button 
              onClick={() => setMode(AppMode.LESSON_PLANNER)}
              className={`px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                mode === AppMode.LESSON_PLANNER 
                  ? 'bg-teal-600/30 text-teal-200 shadow-sm border border-teal-500/30' 
                  : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
            >
              Planner
